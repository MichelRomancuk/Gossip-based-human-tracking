...
# Load all face encodings from the "faces" folder
    ...
    for file_name in os.listdir(folder_path):
        if file_name.endswith(".jpg") or file_name.endswith(".jpeg") or file_name.endswith(".png"):
            ...
            try:
                ...
                known_names.append(os.path.splitext(file_name)[0])  # Save the name without extension
            except IndexError:
    ...
    while True:
        ...
        try:
            ...
            for i, match in enumerate(results):
                if match:
                    local_gossip = f"{known_names[i]};{socket.gethostname()};1"
                    if check_new(local_gossip):
                        write_gossip(local_gossip)
                    break
            time.sleep(1)
